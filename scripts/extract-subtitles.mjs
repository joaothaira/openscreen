...
async function extractAudio(videoPath, outputPath) {
	...
	try {
		execSync(`ffmpeg -y -i "${videoPath}" -vn -acodec pcm_s16le -ar 16000 -ac 1 "${outputPath}"`, {
			stdio: "pipe",
		});
...
function formatAsJSON(
	...
	const _lastFrame = items.length > 0 ? items[items.length - 1].endFrame : 90;
...
`);
	...
	try {
		...
		if (splitAudio && jsonFormat) {
			...
			const sentences = groupIntoSentences(subtitles);
			sentenceAudioFiles = await splitAudioIntoSentences(
				audioPath,
				subtitles,
				sentences,
				sessionDir,
